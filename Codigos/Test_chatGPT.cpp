...
void actualizarGrid(vector<vector<int>>& grid, int L, float probEvaluacion, float probRegreso) {
    ...
    for (int i = 0; i < L; ++i) {
        for (int j = 0; j < L; ++j) {
            ...
            if (grid[i][j] == ACTIVO_RIESGO && vecinosSinRiesgo == 0 && contarVecinos(grid, i, j, EVALUACION, L) == 0) {
                nuevoGrid[i][j] = ACTIVO_RIESGO; // Regla 1
            } else if (grid[i][j] == ACTIVO_RIESGO && vecinosSinRiesgo >= 1) {
...
int main() {
    ...
    int L = 9; // Tamaño del grid
    float probEvaluacion = 0.5; // Probabilidad de evaluación
    float probRegreso = 0.2;    // Probabilidad de regreso
    vector<vector<int>> grid(L, vector<int>(L, ACTIVO_RIESGO));
    string nombreArchivo = "resultado.txt";

    for (int i = 0; i < L; ++i) {
        for (int j = 0; j < L; ++j) {
            grid[i][j] = (i*L+j == L*(L-1)/2+(L-1)/2) ? 2 : 0; // Estados 0, 1 o 2
        }
    }
    ...
    // Contar y guardar el estado inicial
